...
class MultiCapperVerifier:
    ...
    def verify_picks(
        ...
        expected_cappers: list[str] | None = None,
        ...
        if expected_cappers:
            return cls._verify_against_expected(unique_cappers, expected_cappers, message_id)
    ...
    def _verify_against_expected(
        cls, actual: list[str], expected: list[str], message_id: int | None
    ) -> CapperVerificationResult:
    ...
    @classmethod
    def generate_reparse_hint(cls, result: CapperVerificationResult, original_ocr: str = "") -> str:
        """Generate a hint for AI to reparse with correct capper attribution."""
        lines = ["### CAPPER ATTRIBUTION CORRECTION NEEDED", "", f"**Issue:** {result.reason}", ""]
...
def verify_all_picks(
    messages: list[dict[str, Any]], all_picks: list[dict[str, Any]]
    ...
    for msg in messages:
        ...
        if len(image_paths) > 1:
            correlation = AlbumCorrelator.extract_and_correlate(caption, image_paths)
