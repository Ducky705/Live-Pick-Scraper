...
async def run_pipeline(target_date: str, sample_size: int = None, use_cache: bool = False):
    ...
    if ocr_tasks:
        ...
        metadata["stages"]["ocr"] = {"time": time.time() - start, "images": len(ocr_tasks)}
    ...
    metadata["stages"]["classify"] = {"input": len(unique_msgs), "selected": len(selected)}
    ...
    try:
        ...
        metadata["stages"]["grade"] = {"wins": wins, "losses": losses, "pending": pending}
