...
def run_enhanced_benchmark(model=None, limit=0):
    ...
    classification_stats = {"PICK": 0, "PROMO": 0, "RECAP": 0, "DATA": 0, "UNKNOWN": 0}
    ...
    for idx, img_key in enumerate(images_to_test):
        ...
        # Create a message list compatible with RuleBasedExtractor
        # It expects: [{'id': ..., 'text': ..., 'ocr_text': ...}]
        rule_msg_input = [
            {
                "id": idx + 1,
                "text": "",  # Golden set usually doesn't have caption separate, we rely on OCR
                "ocr_text": ocr_text,
            }
        ]

        rule_picks, _ = RuleBasedExtractor.extract(rule_msg_input)
        ...
        if rule_picks:
            ...
            # Simulate 0 latency for rules (it's negligible compared to AI)
