...
def run_full_pipeline_benchmark(use_vision_ocr=True, model=None, limit=0, save_name="baseline"):
    ...
    for idx, img_key in enumerate(images_to_test):
        ...
        try:
            ...
            if not ocr_text or len(ocr_text.strip()) < 10:
                logging.warning("  -> Empty OCR result, skipping parsing")
                total_fn += len(expected_picks)
                continue

            # Step 2: Parsing - Use Hybrid Pool with compact prompt
            synthetic_message = {"id": idx + 1, "text": "", "ocr_texts": [ocr_text], "ocr_text": ocr_text}
            ...
            if not response:
                logging.error("  -> No response from pool")
                total_fn += len(expected_picks)
                continue
    ...
    results = {
        "config": {"model": model, "use_vision_ocr": use_vision_ocr, "images_tested": len(images_to_test)},
        "metrics": {
...
if __name__ == "__main__":
    ...
    run_full_pipeline_benchmark(
        use_vision_ocr=not args.no_vision, model=args.model, limit=args.limit, save_name=args.name
    )
