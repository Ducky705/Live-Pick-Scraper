...
def process_item(item):
    ...
    data_input = {"id": item_id, "text": item.get("original_text", ""), "ocr_texts": item.get("ocr_texts", [])}
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(futures):
            ...
            logging.info(
                f"[{status}] ID:{item_id} | Exp: {len(expected_picks)} | Act: {len(actual_picks)} | Match: {matches}"
            )
