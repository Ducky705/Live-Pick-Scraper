...
```mermaid
graph LR
    Pick[Raw Pick] --> Parser[PickParser]
    Parser --> Parsed[Pick Object]
    Parsed --> Engine[GraderEngine]
    Engine --> Matcher[Game Matcher]
    Matcher --> Scores[ESPN Scores]
    Engine --> Logic[Grading Logic]
    Logic --> Result[GradedPick]
```
