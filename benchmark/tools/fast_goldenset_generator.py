...
def extract_picks_heuristic(text):
    ...
    p1 = re.findall(
        r"(?:(\d+(?:%|u|\*|U))\s*(?:-|–)?\s*)([A-Za-z0-9\s&]+(?:(?:\+|-)\d+(?:\.5)?|ML|Over\s*\d+|Under\s*\d+|O\s*\d+|U\s*\d+))",
        text,
        re.IGNORECASE,
    )
...
def synthesize_variation(original_msg):
    ...
    sport = "NBA"  # Default
    if "NHL" in text or "Goal" in text:
        sport = "NHL"
    elif "NFL" in text or "Touchdown" in text:
        sport = "NFL"
    elif "UFC" in text or "Fight" in text:
        sport = "UFC"
    elif "NCAAB" in text or "CBB" in text:
        sport = "NCAAB"
