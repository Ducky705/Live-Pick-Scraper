...
-----------------------
Performs a read-only validation of the entire pipeline:
1. Scrape data from Yesterday (ET) from all sources (Telegram, Twitter, Discord).
2. Run Extraction Pipeline (AI).
3. Grade the picks against live scores.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(DATA_DIR, "logs", "dry_run_analysis.log"), encoding="utf-8")
    ]
)
...
def generate_report(target_date, messages, picks, graded_picks, stats, time_taken):
    """Generate Markdown report."""
    report_path = os.path.join(OUTPUT_DIR, f"dry_run_analysis_{target_date}.md")
    
    total_graded = stats["WIN"] + stats["LOSS"] + stats["PUSH"]
    accuracy = (stats["WIN"] / total_graded * 100) if total_graded > 0 else 0.0
    
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"# Dry Run Analysis Report - {target_date}\n\n")
        ...
        f.write("## 3. Performance Breakdown\n")
        f.write(f"- **Extraction Yield:** {len(picks) / len(messages) * 100:.2f}% (Picks/Messages)\n")
        f.write("\n")
        
        f.write("## 4. Missed/Error Cases (Diagnosis)\n")
        errors = [p for p in graded_picks if p.get("grade") in ["ERROR", "PENDING"]]
        if errors:
            f.write(f"Found {len(errors)} items that could not be graded definitively.\n\n")
            f.write("| Pick | League | Reason/Details |\n")
            f.write("| :--- | :--- | :--- |\n")
            for e in errors:
                reason = e.get("grading_details") or e.get("grading_error") or "Unknown"
                pick_clean = str(e.get('pick')).replace("|", r"\|")
                f.write(f"| {pick_clean} | {e.get('league')} | {reason} |\n")
        else:
            f.write("No grading errors found.\n")
            
        f.write("\n## 5. Detailed Pick List\n")
...
async def main():
    ...
    if os.path.exists(cache_path):
        print(f"Found cache: {cache_path}. Loading picks...")
        with open(cache_path, "r") as f:
            picks = json.load(f)
    else:
        try:
            # Use ExtractionPipeline
            # Note: pipeline does some validation/dedup internally
    ...
    report_file = generate_report(target_date, messages, picks, graded_picks, stats, total_time)
