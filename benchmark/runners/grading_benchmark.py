...
TEST_PICKS = [
    ...
    {
        "pick": "(NFL) Jalen Hurts: RushYds Over 48.5 / (NFL) A.J. Brown: RecYds Over 80.5",
        "league": "NFL",
        "expected_type": "Parlay",
    },
    # TENNIS SPECIAL
...
def benchmark_parser():
    ...
    for pick in TEST_PICKS:
        ...
        results.append({"pick": pick["pick"][:50], "expected": expected, "detected": detected, "correct": is_correct})
...
def benchmark_grading_coverage():
    ...
    try:
        with open("cache/graded_picks.json") as f:
            cached = json.load(f)
...
def benchmark_speed():
    ...
    synthetic = [{"pick": f"Team{i} -5.5", "league": "NBA"} for i in range(100)]
...
def run_full_benchmark():
    ...
    results = {"timestamp": datetime.now().isoformat(), "parser_accuracy": 0, "coverage": 0, "speed_ms": 0}
