...
# Pre-compute inverted index for O(1) lookup
# Order matters: simpler names can be overridden by more specific ones if needed,
# but here we just map name -> league.
# We handle collisions by prioritizing Major Leagues in a specific order if needed,
# but mostly names are unique or valid for multiple (e.g. "Rangers" in NHL and MLB).
# For now, we simply iterate and overwrite, effectively last-writer-wins.

ENTITY_TO_LEAGUE_MAP = {}

# Load mapping (lower case)
for team in NBA_TEAMS:
    ENTITY_TO_LEAGUE_MAP[team.lower()] = "NBA"
for team in NFL_TEAMS:
    ENTITY_TO_LEAGUE_MAP[team.lower()] = "NFL"
for team in NHL_TEAMS:
    ENTITY_TO_LEAGUE_MAP[team.lower()] = "NHL"
for team in COLLEGE_TEAMS:
    # Don't overwrite professional teams (e.g. if a college has same name)
    if team.lower() not in ENTITY_TO_LEAGUE_MAP:
        ENTITY_TO_LEAGUE_MAP[team.lower()] = "NCAAB"

# Special handle for specific cross-sport ambiguities if any
# e.g. "Giants" (NFL/MLB), "Jets" (NFL/NHL), "Kings" (NBA/NHL)
# Current logic: Last one loaded wins for exact match.
# NHL loaded after NBA -> Kings = NHL.
# NHL loaded after NFL -> Jets = NHL.
# Should we support lists? The usage expects a single string or None.
# For betting context, usually context resolves it, but here we just need a decent guess.


def infer_league_from_entity(entity: str) -> str | None:
    """
    Infer league from a team/player name using specialized O(1) lookup.
    """
    if not entity:
        return None
    ...
    if entity_lower in ENTITY_TO_LEAGUE_MAP:
        return ENTITY_TO_LEAGUE_MAP[entity_lower]

    # 2. Tennis check (Set lookup, O(1) average)
    if is_tennis_entity(entity_lower):
        return "TENNIS"

    # 3. Substring fallback (Slower, only if exact match fails)
    # This catches "Los Angeles Lakers" if "Lakers" is the key.
    # Optimization: iterate through words?
    # Or just check if known keys are in the string?
    ...
    if len(entity_lower) > 3:
        import difflib
        # Get list of all known keys
        keys = list(ENTITY_TO_LEAGUE_MAP.keys())
        # Use cutoff=0.8 (allows 1-2 char errors for typical team names)
        # O(N) but N is small (~200) and difflib is optimized C
        matches = difflib.get_close_matches(entity_lower, keys, n=1, cutoff=0.8)
        if matches:
