...
class AlbumCorrelator:
    ...
    def extract_and_correlate(
        cls, caption: str, image_paths: list[str], channel_name: str | None = None
    ) -> CapperImageMapping:
        """
    ...
    @classmethod
    def _correlate_names_to_images(cls, names: list[str], image_count: int) -> tuple[dict[int, str], float]:
        """
...
def generate_album_context(
    caption: str, ocr_texts: list[str], image_paths: list[str], channel_name: str | None = None
) -> str:
    """
    ...
    if len(correlation.capper_names) != correlation.image_count:
        lines.append("")
        lines.append(
            f"**WARNING:** Name count ({len(correlation.capper_names)}) != image count ({correlation.image_count}). Use visual cues for unmapped images."
        )

    lines.append("")
    lines.append(
        "**INSTRUCTION:** When outputting picks, use the capper name from this mapping for each [OCR N] block."
    )
