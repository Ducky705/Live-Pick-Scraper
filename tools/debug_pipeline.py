...
def validate_pick_format(picks: list) -> dict:
    ...
    for i, pick in enumerate(picks):
        ...
        if lg not in VALID_LEAGUES:
            issues["invalid_league"].append({"id": pick_id, "got": lg, "pick": p})

        # 2. Validate type
        if ty not in VALID_TYPES:
            issues["invalid_type"].append({"id": pick_id, "got": ty, "pick": p})
        ...
        if ty == "Spread" and any(ind.lower() in p.lower() for ind in period_indicators):
            issues["period_misclassified"].append({"id": pick_id, "pick": p, "should_be": "Period"})

        # 4. Moneyline format: Should end with "ML"
        if ty == "Moneyline" and not p.strip().upper().endswith("ML"):
            issues["moneyline_missing_ml"].append({"id": pick_id, "pick": p, "expected": f"{p} ML"})
        ...
        if ty == "Spread":
            ...
            if lg == "TENNIS":
                ...
                if not any(ind.lower() in p.lower() for ind in period_indicators):
                    issues["spread_bad_format"].append({"id": pick_id, "pick": p, "expected": "Team Name +/-X.X"})
    ...
    if s["invalid_league_count"] == 0:
        log_success("League values: All valid")
    else:
    ...
    if s["invalid_type_count"] == 0:
        log_success("Type values: All valid")
    else:
...
def load_cache(stage: str) -> dict | None:
...
def get_sample_size(args) -> int | None:
        ...
        if picks:
            ...
            for i, pick in enumerate(picks[:5]):
                ...
                if source_item:
                    ...
                    if caption:
                        print(
                            f"{Colors.YELLOW}[CAPTION]{Colors.RESET}\n{caption[:200]}{'...' if len(caption) > 200 else ''}"
                        )
                    ...
                    if ocr_texts:
                        ...
                        for idx, text in enumerate(ocr_texts):
                            print(
                                f"  Img {idx + 1}: {text[:300].replace(chr(10), ' ')}{'...' if len(text) > 300 else ''}"
                            )
...
def main():
    ...
    parser.add_argument(
        "stage", choices=["fetch", "ocr", "parse", "grade", "report", "full"], help="Pipeline stage to run"
    )

    parser.add_argument("--fresh", action="store_true", help="Ignore cache, re-run from scratch")

    parser.add_argument(
        "--sample", default="quick", help="Sample size: quick (10), medium (30), full (all), or a number"
    )
