...
def fetch_pending_picks(supabase, limit=None):
    ...
    while True:
        query = (
            supabase.table("picks")
            .select("id, pick_value, pick_date, result, grading_notes, league_id")
            .not_.in_("result", graded_results)
            .order("pick_date", desc=True)
            .range(offset, offset + batch_size - 1)
        )
...
def grade_single_pick(pick, league_map):
    ...
    try:
        ...
        if not scores:
            return pick_id, {"grade": "PENDING", "score_summary": "No scores available for this date", "details": ""}
        ...
        return pick_id, {"grade": "ERROR", "score_summary": "", "details": str(e)[:200]}
...
def grade_picks_parallel(picks, league_map, max_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(grade_single_pick, pick, league_map): pick["id"] for pick in picks}
...
def main():
    ...
    updates_to_apply = {pick_id: data for pick_id, data in results.items() if data["grade"] in ("WIN", "LOSS", "PUSH")}
