...
logger = logging.getLogger("BenchmarkPlatinum")
...
def fuzzy_match(expected, actual):
    ...
    stop_words = {"the", "a", "an", "bet", "pick", "prediction", "of", "in", "ml", "moneyline", "spread", "total", "over", "under"}
    ...
    if not exp_tokens:
        ...
        pick_match = (ratio >= 0.6) or (exp_pick_raw in act_pick_raw) or (act_pick_raw in exp_pick_raw)
