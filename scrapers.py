...
# --- Local Imports ---
from config import (
    TELEGRAM_API_ID, TELEGRAM_API_HASH, TELEGRAM_SESSION_NAME, TELEGRAM_CHANNELS,
    SCRAPE_WINDOW_HOURS, EASTERN_TIMEZONE, PICK_STATUS_PENDING
)
...
async def scrape_telegram():
    ...
    try:
        ...
        after_time_utc = datetime.now(timezone.utc) - timedelta(hours=SCRAPE_WINDOW_HOURS)
        ...
        for channel_entity in TELEGRAM_CHANNELS:
            try:
                ...
                capper_name = getattr(entity, 'title', str(channel_entity)) 
                logging.info(f"Scraping Telegram channel: {capper_name}")
