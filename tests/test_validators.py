...
def test_multi_pick_validator():
    ...
    print("Pick count estimation:")
    print("  OCR text has 4 picks")
    ...
    parsed_picks = [{"message_id": 1, "pick": "Lakers -5", "capper_name": "Test"}]  # Only 1 pick extracted

    result = MultiPickValidator.validate_extraction(ocr_text=ocr_text, parsed_picks=parsed_picks, message_id=1)
...
def test_multi_capper_verifier():
    ...
    print("Merged name detection:")
    print("  Picks have capper: 'KingCap, BetMaster' (merged)")
    ...
    result2 = MultiCapperVerifier.verify_picks(
        picks=picks_valid, expected_cappers=["KingCap", "BetMaster"], ocr_block_count=2
    )
    print("\nExpected capper verification:")
    print("  Expected: ['KingCap', 'BetMaster']")
...
def test_decoder_full_pipeline():
    ...
    print("Spread misclassification fix:")
