...
def build_message_id_map(scraper_picks: List[Dict]) -> Dict[Any, List[Dict]]:
    ...
    for pick in scraper_picks:
        ...
        if msg_id:
            # Normalize to string for reliable lookup
            by_message[str(msg_id)].append(pick)
...
def compare_message(judgment: Dict, scraper_picks: List[Dict]) -> Dict:
    ...
    """
    msg_id = str(judgment.get("message_id"))
...
def run_comparison(golden_set: Dict, scraper_picks: List[Dict]) -> Dict:
    ...
    results["metrics"] = {
        ...
        "precision": round(
            ...
            / max(len([c for c in results["comparisons"] if c["scraper_picks"]]), 1)
            * 100,
            2,
        ),
...
def main():
    ...
    if isinstance(golden_set, list):
        ...
        # Map 'expected_picks' to 'picks' and 'has_picks'
        for j in judgments:
            if "expected_picks" in j:
    ...
    # run_comparison expects a dict with 'judgments' key

    golden_set_wrapper = {"judgments": judgments}
    results = run_comparison(golden_set_wrapper, scraper_picks)
