...
def check_for_updates(github_token=None):
    ...
    if not token:
        return {"update_available": False, "error": "No GitHub token configured"}
    ...
    try:
        ...
        for asset in assets:
            ...
            if system == "windows":
                ...
                if (
                    ...
                    or ".exe" in name
                    or "win" in name
                    or "telegramscr" in name
                    or "cappersuite" in name
                ):
        ...
        return {"update_available": False, "error": "Update check timed out"}
    except requests.exceptions.RequestException as e:
        return {"update_available": False, "error": f"Network error: {e!s}"}
...
"""
    ...
    subprocess.Popen(["cmd", "/c", batch_script], startupinfo=startupinfo, creationflags=subprocess.CREATE_NO_WINDOW)
...
"""
    ...
    subprocess.Popen(
        ["/bin/bash", script_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True
    )
